...
- `frontend_env=auto|local|staging` (default `auto`) to forward browser-validation environment choice into `/compound-engineering-core:workflows:frontend-validate`
...
3. If frontend/session/state surfaces changed, run `/compound-engineering-core:workflows:frontend-validate "<pr-number-or-url> sha=<head-sha> env=<frontend_env|auto> playwright=auto"` automatically from this workflow using the current head SHA.
...
6. If PR head SHA changes at any time (new push/force-push), invalidate prior gate result and rerun required gates on the new head SHA before making merge decisions.
   - For frontend/session/state changes, rerun `/compound-engineering-core:workflows:frontend-validate "<pr-number-or-url> sha=<new-head-sha> env=<frontend_env|auto> playwright=auto"` first.
