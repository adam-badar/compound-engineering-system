...
9. For frontend/session/state changes, current-SHA browser validation evidence exists and passes (or is explicitly `N/A`)
...
- `/workflows:work` should auto-run `/workflows:frontend-validate` before PR review when frontend/session/state surfaces changed.
- On any later SHA change for qualifying frontend PRs, `/workflows:work` should rerun frontend validation on the new SHA before rerunning PR review.
