...
- **Execution mode:** agent teams are required for this workflow
...
### 1.4 Agent teams preflight (required)

Before any review loop:

1. Validate agent teams are enabled (`CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1`).
2. Validate teammate fan-out is available for `plan_review_agents`.
3. If unavailable, fail closed:
...
2. Run teammate plan-review agents in parallel via agent teams.
