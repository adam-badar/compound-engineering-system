...
- `repo-research-analyst` (existing patterns and likely touchpoints)
- `learnings-researcher` (historical failures and prior fixes)

Conditionally run based on issue profile:

- `security-sentinel` for auth/trust/input risks
- `performance-oracle` for latency/throughput/resource symptoms
- `architecture-strategist` for boundary/control-flow failures
- `kieran-python-reviewer` for Python runtime/typing/async defects
...
`/workflows:work <plan-or-task-path>`
